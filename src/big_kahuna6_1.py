...
8/2/17 BCM - Allow to be used for CMIP and AMIP paths at the same time 6.1

---THIS SCRIPT TAKES OLD CMIP/AMIP DATA AND REPROCESS IT WITH CMOR324 TO PRODUCE IT IN THE FORMAT OF CMIP6---
THIS SCRIPT TAKES A SET OF OLD AMIP AND CMIP DATA (PATHS SPECIFIED BY USER), CREATES THE REQUIRED USER_INPUT.JSON FILE BY ATTEMPTING
...
THE CODE ALSO MAKES EXCEPTION FOR CERTAIN ALIASES AND VARIABLES THAT HAVE TROUBLESOME DATA. THE CODE WILL NOT PROCESS A VARIABLE
...
AND NONE OF ITS DATA IS REPROCESSED.
...
@author: musci2
"""
import cmor, gc, json, sys, os, shutil, random, glob ; # math, difflib
#import vcs
import cdms2 as cdm
...
user = os.getenv('USER')
sys.path.insert(0,os.path.join(homeDir,'git/CMIP2_CVs/src'))
from kludgersTwo import kludgers
from Variable_Table_Matcher import Variable_Table_Matcher
from matplotlib.backends.backend_pdf import PdfPages
gc.collect()

DataPathin1 = '/oldCMIPs/PJG_StorageRetrieval/AMIP2-STORAGE/mo/'
DataType1 = 'AMIP'
DataPathin2 = '/oldCMIPs/PJG_StorageRetrieval/CMIP6-STORAGE/mo/'
DataType2 = 'CMIP' 
paths = [DataPathin1,DataPathin2]
types = [DataType1,DataType2]

#specify run of interst: CONTROL or PERTURBED
for jh,path in enumerate(paths):
    if 'AMIP' in types[jh]:
        explst = ['AMIP']
        sourceIdjson = 'AMIP1&2_source_id.json'
    elif 'CMIP' in types[jh]:
        explst = ['con','per']
        sourceIdjson = 'CMIP1&2_source_id.json'
        
    for exp in explst: #'con','per'
        ...
        pathin1= path  ## PATH FOR MAIP INPUT DATA
        ...
        savepath = os.path.join('/oldCMIPs',user)
        ...
        #%%
        print ''
        print '1. MATCH VARIABLES TO JSON TABLES'
        # Call new function to determine what variables in path have matching  spots in the json tables
        dtype = types[jh]
        ...
        for Vcount,gh in enumerate(variables): #84 = tas, #41 = pr, 113 = tos
            ...
            if TableID in ['Missing']:
                print 'Skipping variable b/c no matching varb in json tables'
                continue
    
            #%% INPUT JSON FILE CREATION WITH LOOP
            print ' '
            print '2. SET PATH AND AQUIRE DATA LIST'
            ...
            #%%
            print ' '
            print '3. CREATING USER INPUT JSON'
            # make QC dict
            QualCon = {}
            QualCon[varb]={}
            bounds = list()
            ...
            with open(os.path.join(homeDir,'git/CMIP2_CVs/',sourceIdjson), 'r') as f: # SOURCE ID FOR AMIP RUNS
    ...
    #                     break
                 #for alias in ['gfdl-c04a']:
                     ...
                     if alias in LIST:
                         ...
                         common_user_input_dict['activity_id']=types[jh] # required by CMIP6_CV.json
                         activity_id = common_user_input_dict['activity_id']
            ...
            # converts each dictionary into individual user_input_json files for the variable and each model inside the variable, saves to user specified folder
                     ...
                     print ' '
                     print '4. CHECK TO SEE IF ALIAS OR VAR IS IN THE TROUBLESOME/EXCEPTION BUCKET -- SKIP IF SO'
                     ...
                     if alias in ['derf-95a','mpi-96a']:
                         ...
                         print 'Time bounds have gaps between them'
                         continue
                     elif alias in ['derf-98a']: #and var in ['ta','ua','va','wap','hur']:
                         QualCon[var][alias]=['Error: axis longitude has bounds values spanning more than 360 degrees']
                         print 'Error: axis longitude has bounds values spanning more than 360 degrees'
                         continue
                     elif alias in ['ukmo-98a'] and var in ['cl','cli','clw']:
                         QualCon[var][alias]=['Udunits error becuase d.getLevel() returns none type. the level axis is labeled as da18']
                     ...
                     print ' '
                     print '5. OBTAIN XML OF INTEREST'
                     lstall = ' '
                     ...
                     print ' '
                     print '6. LOAD CMOR TABLE AND AXES'
...
#                         table_ = "Omon"
                     ...
                     print ' '
                     print '7. PERFOMRING KLUDGERS'
    
                     returns = kludgers(var, d, axis_ids, alias, table )
                     uniMsg = returns[0]
                     d.units = returns[1]
                     varid = returns[2]
                     Converter =returns[3]
                     ...
                     print ' '
                     print '9. QUALITY CONTROL STARTING ---REOPENING CMOR'
            ...
            ## GLOBAL AVG
                     ...
                     QualCon[var][alias]={}
                     QualCon[var][alias]['PreCMOR'] = ['Global Avg: '+str(PreGblAvg)+' '+oldUnits,'Min: '+ str(PreCmor.min()),'Max: ' + str(PreCmor.max()),' LowestLev: '+str(levLow)+' '+levUni,'  '+BoundsIssue,' Variable list position:'+str(variables.index(gh)),' Alias list position:'+str(lst.index(alias))]
            ...
            #%% PLOTTING
            print ' '
            print '10. CREATING PLOTS FOR QC'
           ...
           ## 4D Plotting
            ...
            if max(boundsSeas) > 1e19: #reomve missing data that shows up as a max
                ...
                remove = boundsSeas.index(min(boundsSeas))
                del boundsSeas[remove]
            ...
            #%% FILE CREATION
            print ' '
            print '11. CREATING AND SAVING FILES'
...
#            print >> ff, jj
#            ff.close()
            with open(completePath, 'w') as ff:
                json.dump(QualCon, ff, sort_keys=True, indent=4, ensure_ascii=True,separators=(',',':'))
